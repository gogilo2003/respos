...
<template>
  ...
  >
    ...
    >
      ...
      <span
        v-if="!collapsed"
        class="text-sm font-bold tracking-wide text-neutral-900 whitespace-nowrap"
      >
        WEBSITE LOGO
      </span>
    </div>
      ...
      <div v-for="item in items" :key="item.key" class="relative">
        ...
        >
          ...
          <span
            ...
            :class="[badgeClass(item), collapsed ? 'right-1/2 translate-x-1/2' : '-right-6']"
          >
            <component :is="ICONS[item.icon]" class="h-5 w-5 text-white" aria-hidden="true" />
          </span>
        </component>
        ...
        >
          <component :is="ICONS[item.icon]" class="h-5 w-5 shrink-0 text-current" aria-hidden="true" />
    ...
    <!-- Footer -->
    <div class="border-t border-neutral-100 px-8 pt-6" :class="{ 'px-0': collapsed }">
      <div
        v-if="user"
      ...
      >
        <img
          ...
          :alt="user.name"
        />
        <div v-else class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-neutral-200 text-xs font-semibold text-neutral-600">
          {{ user.name.slice(0, 1) }}
        </div>
        <span v-if="!collapsed" class="text-sm font-semibold tracking-wide text-neutral-800 whitespace-nowrap">
      ...
      <Link
        ...
        method="post"
        as="button"
        class="hover-row flex items-center gap-4 pt-3 pb-2 pl-8 -ml-8 text-neutral-500"
        :class="collapsed ? 'w-full justify-center pl-0 ml-0' : ''"
      >
        <ArrowRightOnRectangleIcon class="h-5 w-5 shrink-0 text-current" aria-hidden="true" />
        <span v-if="!collapsed" class="text-sm font-semibold tracking-wide">LOGOUT</span>
      </Link>
    </div>
    ...
    <button
      ...
      @click="toggle"
    >
      <svg viewBox="0 0 24 24" class="h-3.5 w-3.5 stroke-neutral-600 fill-none transition-transform" :class="{ 'rotate-180': collapsed }" stroke-width="2">
